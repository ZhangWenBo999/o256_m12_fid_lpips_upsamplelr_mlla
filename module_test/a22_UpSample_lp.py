...
class DySample_UP(nn.Module):
    ...
    def sample(self, x, offset):
        ...
        coords = F.pixel_shuffle(coords.contiguous().view(B, -1, H, W), self.scale).contiguous().view(B, 2, -1, self.scale * H, self.scale * W).permute(0, 2, 3, 4, 1).contiguous().flatten(0, 1)
